...
class auth extends \auth_plugin_base {
    ...
    public function complete_login(client $client, $redirecturl) {
        ...
        if (empty ($userinfo['email'])){
            $userinfo['email'] = $userinfo['username']."_noreply@india.benesse.com";
        }
        ...
        // We used to call authenticate_user - but that won't work 
        // if the current user has a different default authentication
        ...
        // Enrol the user to specific course created from school, subject 
        // and grade name.
        if($is_Student){ 
                $this->enrol_user_to_course($rawuserinfo, $user->id);
        } else{
            $this->enrol_teacher_to_courses($rawuserinfo, $user->id);
        }
    ...
    protected function enrol_teacher_to_courses($userInfo, $userId)
    {
        global $DB;

        $userInfo = (array) $userInfo;

        $school_name = str_replace(' ','_',$userInfo['schools'][0]->name);
            ...
            foreach($grades as $grade) {
                foreach ($years as $year) {
                    foreach ($terms as $term) {
                        ...
                        $course_name = $subject . '_Grade_' . $grade . '_' . $term . '_' . $school_name;

                        $course = $DB->get_record('course', ['shortname' => $course_name], 'id');
    ...
    ////////////////////////////
    protected function enrol_user_to_course($userInfo, $userId)
    {
        global $DB;

        $userInfo = (array) $userInfo;
        $grade = $userInfo['currentClassroom']->grade;
        $school_name = str_replace(' ','_',$userInfo['schools'][0]->name);
