...
//  Display the course home page.
    ...
    $IMG_url = '';
    $Gcontext = context_course::instance( $COURSE->id );
       $fs = get_file_storage();
       $files = $fs->get_area_files( $Gcontext->id, 'course', 'overviewfiles', 0 );

       foreach ( $files as $f )
       {
         if ( $f->is_valid_image() )
         {
            $IMG_url = moodle_url::make_pluginfile_url( $f->get_contextid(), $f->get_component(), $f->get_filearea(), null, $f->get_filepath(), $f->get_filename(), false );
         }
       }

       if(!empty($IMG_url)){
       ?>
       <style>.dp_course_header {background: url('<?=$IMG_url?>') no-repeat; background-size: 100%; height:250px;}</style>
       <?php }
    echo $OUTPUT->header();
    ...
    if($rolename !== "student"){ echo "<style> .progress, .prog_span {display:none;} </style>"; }
    //end@dp
