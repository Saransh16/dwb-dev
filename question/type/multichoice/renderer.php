...
abstract class qtype_multichoice_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
		...
		$Co_response = $this->correct_response($qa);
		if(!$options->rightanswer){
		$onlyAns = end(explode(':',strip_tags($Co_response)));
		//echo '<br>'.end(explode(':', );
		//echo '<br>';
		}
        ...
        foreach ($question->get_order($qa) as $value => $ansid) {
            ...
            if ($options->correctness && $isselected) {
                ...
                //$feedbackimg[] = html_writer::span($this->feedback_image($this->is_right($ans)), 'ml-1'); //karubaki@dp
		...
		/*	
			//karubaki@dp
			$choicetext1 = strip_tags($choicetext);
			$getFullUrl = basename($_SERVER['REQUEST_URI']);
			if ((strpos($getFullUrl,'scrollpos')) !== false){
				if((!empty($onlyAns))&& (trim($choicetext1) == trim($onlyAns))){
					$class = $class.' mk_correct';	
				}
			}
			
			if(strpos($class,'incorrect') !== false){
			$class = $class.' disp_wrong';	
			}
			//end@dp
		 */
    //karubaki@dp
			...
			$getFullUrl = basename($_SERVER['REQUEST_URI']);
			if ((strpos($getFullUrl,'scrollpos')) !== false){
				if((!empty($onlyAns))&& (trim($choicetext1) == trim($onlyAns))&& ($response != -1)){
					$class = $class.' mk_correct';
				}
			}
			else if ((strpos($getFullUrl,'review')) !== false){
				$onlyAns1 = end(explode(':',strip_tags($Co_response)));
				if((!empty($onlyAns1))&& (trim($choicetext1) == trim($onlyAns1)))
				$class = $class.' mk_correct';
			}
			if(strpos($class,'incorrect') !== false) {
                $class = $class.' disp_wrong';
